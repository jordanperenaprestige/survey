...
class MainController extends AppBaseController
{
    ...
    public function storeConcern(Request $request)
    {
        ...
        foreach (explode(",", $request->concern) as $v) {
            ...
            switch ($answer->sms_recepient) {
                ...
                default:
                    $it[] = $answer->answer;
                    $it_floor_room[] = $floor->name . '/' . $rooms->name;
            }
        }
        ...
        foreach ($users as $vuser) {
            switch ($vuser->role) {
                case 9:
                    $role = $hk;
                    $role_name = 'HK';
                    $role_floor_room = $hk_floor_room[0];
                    break;
                case 10:
                    $role = $mst;
                    $role_name = 'MST';
                    $role_floor_room = $mst_floor_room[0];
                    break;
                default:
                    $role = $it;
                    $role_name = 'IT';
                    $role_floor_room = $it_floor_room[0];
            }
            if (count($role) > 0) {
                $concern = implode(",", $role);
                $message = $role_name . ' - ' . $role_floor_room . ' - Concern: ' . $concern . ' ' . date("Y-m-d h:i:sa");
                $target = $vuser->mobile;
                $sender_id = $vuser->id;
    ...
    public function updateStatus(Request $request)
    {
        try {
            ...
            QuestionnaireSurvey::where('questionnaire_id', $questionnaire_survey->questionnaire_id)
            ->where('questionnaire_answer_id', $questionnaire_survey->questionnaire_answer_id)
            ->where('site_id', $questionnaire_survey->site_id)
            ->where('site_building_id', $questionnaire_survey->site_building_id)
            ->where('site_building_level_id', $questionnaire_survey->site_building_level_id)
            ->where('site_building_room_id', $questionnaire_survey->site_building_room_id)
            ->update($data);
