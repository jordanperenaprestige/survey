...
use App\Http\Controllers\Admin\Interfaces\QuestionnaireSurveyControllerInterface;
...
class QuestionnaireSurveysController extends AppBaseController implements QuestionnaireSurveyControllerInterface
{
    /************************************
     * QUESTIONNAIRE SURVEYS MANAGEMENT *
     ************************************/
    ...
    {
        return view('admin.questionnaire_survey');
    }

    public function list(Request $request)
    { 
        try {
            $surveys = QuestionnaireSurveyViewModel::when(request('search'), function ($query) {
                return $query->where('name', 'LIKE', '%' . request('search') . '%');
            })
                ->latest()
                ->paginate(request('perPage'));

            return $this->responsePaginate($surveys, 'Successfully Retreived!', 200);
        } catch (\Exception $e) {
            return response([
    ...
    {
        try {
            $reservation = QuestionnaireSurveyViewModel::find($id);
